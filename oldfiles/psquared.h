...
typedef struct PSquared
{
	...
	// Last k value
	int lastK;
	void (*PSq)(float, struct PSquared *);
	void (*init)(struct PSquared *);
	bool (*acceptInitial)(float, struct PSquared *);
	float (*initialSetPercentile)(struct PSquared *);
	float (*accept)(float, struct PSquared *);
	float (*getPValue)(struct PSquared *);
	float (*linear)(int , int , struct PSquared *);
	float (*parabolic)(int , int , struct PSquared *);
	void (*dump)(struct PSquared *);
}PSquared;

void PSq(float f, struct PSquared * pq)
{
	// Set percentile
	int i;
	for(i=0;i<5;i++)
...
bool acceptInitial(float x, struct PSquared * pq)
{
	int u,v;
	float a;
...
float initialSetPercentile(struct PSquared * pq)
{
...
float accept(float x, struct PSquared * pq)
{
	int i;
...
float getPValue(struct PSquared * pq)
{
	return pq->pValue;
}

float linear(int d, int i, struct PSquared * pq)
{
	return pq->q[i] + (float)d * (pq->q[i + d] - pq->q[i]) / (float)(pq->n[i + d] - pq->n[i]);
}

float parabolic(int d, int i, struct PSquared * pq)
{
